...
#fold a fold validation
from sklearn.model_selection import KFold, cross_val_score
kf = KFold(n_splits=5, shuffle=True, random_state=42)
scorer = make_scorer(r2_score)
r2_folds = cross_val_score(best_model, X_poly, y, scoring=scorer, cv=kf, n_jobs=-1)
print("R² por fold:", np.round(r2_folds, 3))
print(f"Media R² = {r2_folds.mean():.3f} |Desviación típica = {r2_folds.std():.3f}")
print("Estabilidad (std < 0.05):", "OK" if r2_folds.std() < 0.05 else "ATENCIÓN")
# Gráfico de estabilidad
plt.figure(figsize=(6,4))
folds = np.arange(1, len(r2_folds)+1)
plt.plot(folds, r2_folds, marker='o')
plt.xticks(folds)
plt.xlabel('Fold')
plt.ylabel('R²')
plt.title('Variabilidad de R² – XGBoost Regressor')
plt.grid(True)
plt.tight_layout()
plt.show()
